...
public class Activity_ImportComics_obsolete extends AppCompatActivity {
    ...
    //Global Variables:
    private Activity_ImportComics_obsolete.ImportResponseReceiver importResponseReceiver;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        IntentFilter filter = new IntentFilter(Activity_ImportComics_obsolete.ImportResponseReceiver.IMPORT_ACTION_RESPONSE);
        filter.addCategory(Intent.CATEGORY_DEFAULT);
        importResponseReceiver = new Activity_ImportComics_obsolete.ImportResponseReceiver();
        ...
        try {
            ...
            || (requestCode == REQUEST_CODE_GET_IMPORT_FILE && resultCode == Activity.RESULT_OK))
                ...
                if ((ContextCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                    ...
                    gIntentImport = new Intent(this, Service_ImportComics_obsolete.class);

                    //Put the import method into the intent.
                    if(requestCode == REQUEST_CODE_GET_IMPORT_FOLDER) {
                        gIntentImport.putExtra(Service_ImportComics_obsolete.IMPORT_METHOD, Service_ImportComics_obsolete.IMPORT_METHOD_FOLDER);
                    } else {
                        gIntentImport.putExtra(Service_ImportComics_obsolete.IMPORT_METHOD, Service_ImportComics_obsolete.IMPORT_METHOD_FILE);
                    }
                    ...
                    gIntentImport.putExtra(Service_ImportComics_obsolete.IMPORT_URI, sUriString);
    ...
    public class ImportResponseReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            bUpdateLog = intent.getBooleanExtra(Service_ImportComics_obsolete.UPDATE_LOG_BOOLEAN,false);
            bUpdatePercentComplete = intent.getBooleanExtra(Service_ImportComics_obsolete.UPDATE_PERCENT_COMPLETE_BOOLEAN,false);
            bUpdateStageIndication = intent.getBooleanExtra(Service_ImportComics_obsolete.UPDATE_STAGE_INDICATION_BOOLEAN,false);

            if(bUpdateLog){
                String 		sLogLine;
                sLogLine = intent.getStringExtra(Service_ImportComics_obsolete.LOG_LINE_STRING);
                gtvLog.append(sLogLine);
            }
            if(bUpdatePercentComplete){
                int 		iAmountComplete;
                iAmountComplete = intent.getIntExtra(Service_ImportComics_obsolete.PERCENT_COMPLETE_INT, -1);
                gpbDeterminateBar.setProgress(iAmountComplete);
            }
            if(bUpdateStageIndication){
                String 		sStage;
                sStage = intent.getStringExtra(Service_ImportComics_obsolete.STAGE_STRING);
