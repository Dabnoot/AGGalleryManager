...
public class Activity_ExportComics extends AppCompatActivity {
        ...
        try {
            ...
            if (((requestCode == REQUEST_CODE_GET_EXPORT_SAVE_AS_FILE && resultCode == Activity.RESULT_OK))
                ...
                if ((ContextCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                    ...
                    gIntentExport = new Intent(this, Service_ExportComics.class);
                    gIntentExport.putStringArrayListExtra(Service_ExportComics.ZIP_LIST,galZipList);
                    gIntentExport.putExtra(Service_ExportComics.ZIP_TOTAL_FILE_COUNT,giComicFileCount);

                    Uri uriZipFile = resultData.getData();
                    assert uriZipFile != null;
                    String sUriString = uriZipFile.toString();
                    gIntentExport.putExtra(Service_ExportComics.ZIP_FILE, sUriString);
    ...
    public class ExportResponseReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            bUpdateLog = intent.getBooleanExtra(Service_ExportComics.UPDATE_LOG_BOOLEAN,false);
            bUpdatePercentComplete = intent.getBooleanExtra(Service_ExportComics.UPDATE_PERCENT_COMPLETE_BOOLEAN,false);

            if(bUpdateLog){
                String 		sLogLine;
                sLogLine = intent.getStringExtra(Service_ExportComics.LOG_LINE_STRING);
                gtvExportLog.append(sLogLine);
            }
            if(bUpdatePercentComplete){
                int 		iAmountComplete;
                iAmountComplete = intent.getIntExtra(Service_ExportComics.PERCENT_COMPLETE_INT, -1);
