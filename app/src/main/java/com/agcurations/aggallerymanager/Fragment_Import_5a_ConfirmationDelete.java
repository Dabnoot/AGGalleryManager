 ...
 * Use the {@link Fragment_Import_5a_ConfirmationDelete#newInstance} factory method to
 * create an instance of this fragment.
 */
public class Fragment_Import_5a_ConfirmationDelete extends Fragment {
    ...
    public Fragment_Import_5a_ConfirmationDelete() {
        // Required empty public constructor
    }

    public static Fragment_Import_5a_ConfirmationDelete newInstance() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_import_5a_confirmation_delete, container, false);
    }
    ...
    public void initComponents(){
        ...
        ArrayList<ItemClass_File> alfiFilesToDelete = new ArrayList<>();
        for(ItemClass_File icfFileItem: viewModelImportActivity.alfiConfirmedFileImports){
            if(icfFileItem.bMarkedForDeletion){
                alfiFilesToDelete.add(icfFileItem);
                lStorageSpaceToFree += icfFileItem.lSizeBytes;
            }
        }

        //Populate the ListView with selected file names from an earlier step:
        ListView listView_FilesToDelete = getView().findViewById(R.id.listView_FilesToDelete);
        if (getActivity() == null) {
            return;
        }
        confirmationFileListCustomAdapter = new ConfirmationFileListCustomAdapter(getActivity(), R.id.listView_FilesToDelete, alfiFilesToDelete);
        if(listView_FilesToDelete != null) {
            listView_FilesToDelete.setAdapter(confirmationFileListCustomAdapter);
        }

        //Display the file count:
        TextView textView_FileCount = getView().findViewById(R.id.textView_FileCount);
        String s = Integer.toString(alfiFilesToDelete.size());
        textView_FileCount.setText(s);


        //Display the required space:
        TextView textView_StorageSpaceToFree = getView().findViewById(R.id.textView_StorageSpaceToFree);
        s = GlobalClass.CleanStorageSize(lStorageSpaceToFree, GlobalClass.STORAGE_SIZE_NO_PREFERENCE);
        textView_StorageSpaceToFree.setText(s);
